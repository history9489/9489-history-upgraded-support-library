# ********** 9489 Upgrade History PYP Portal developed on 14th Aug 2026***********
import datetime
import io
import os
import fitz  # PyMuPDF
...
# ==========================================
st.set_page_config(
    page_title="9489 History PYP Portal", 
    page_icon="📜",
    layout="wide"
)
...
# 6 Primary Folders for 9489 History
LOCAL_FOLDERS = {
    "p1_history": "paper1_history",
    "p2_history": "paper2_history",
    "p3_history": "paper3_history",
    "p4_history": "paper4_history",
...
def create_worksheet_docx(basket_items: list) -> io.BytesIO:
    ...
    doc.add_heading(f'PTES {SYLLABUS_CODE} History Worksheet', level=1)
...
if 'has_auto_synced' not in st.session_state:
    ...
    with st.spinner("🚀 Waking up portal & auto-syncing History files via Service Account..."):
        perform_bulk_sync()
...
st.subheader(f"📜 {SYLLABUS_CODE} History PYP Resource Library")
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "🏛️ AS History (P1 & P2)", 
    "🌍 A Level History (P3 & P4)", 
    "🛒 PYP Cart", 
    "🔑 Answer Schemes", 
    "⚙️ Admin / Drive Upload"
])

# --- TAB 1: AS HISTORY (PAPER 1 or PAPER 2) ---
with tab1:
    st.subheader("🏛️ AS Level History Search (Paper 1 & Paper 2)")
    
    selected_as_paper = st.selectbox(
        "Select Target Component Paper:", 
        options=["p1_history", "p2_history"],
        format_func=lambda x: "Paper 1 (Document Question)" if x == "p1_history" else "Paper 2 (Outline Study)",
        key="select_as_paper"
    )

    as_kw = st.text_input(
        "Enter Keywords", 
        placeholder="e.g., Cold War, League of Nations, Bismarck, Liberal Italy, French Revolution", 
        key="as_kw"
    )
...
# --- TAB 2: A LEVEL HISTORY (PAPER 3 or PAPER 4) ---
with tab2:
    st.subheader("🌍 A Level History Search (Paper 3 & Paper 4)")
    
    selected_a_paper = st.selectbox(
        "Select Target Component Paper:", 
        options=["p3_history", "p4_history"],
        format_func=lambda x: "Paper 3 (Interpretations Question)" if x == "p3_history" else "Paper 4 (Depth Study)",
        key="select_a_paper"
    )

    a_kw = st.text_input(
        "Enter Keywords", 
        placeholder="e.g., Holocaust, Origins of Cold War, Mussolini, Stalin, International History", 
        key="a_kw"
    )
...
with tab3:
    ...
    if len(st.session_state.handout_basket) > 0:
        ...
        with st.spinner("Merging selected pages into Word Worksheet..."):
            ...
            target_filename = f"{SYLLABUS_CODE}_History_Worksheet.docx"
        ...
        st.info("🛒 Your Cart is currently empty! Search AS or A Level History tabs and click '➕ Add to Cart' to start building your worksheet.")
...
with tab5:
    ...
    st.caption("Secure admin portal for managing the 6 Google Drive repositories for History 9489.")
    ...
    if pwd_input and pwd_input == admin_pwd:
        ...
        with c1:
            st.link_button("📜 Paper 1 (p1_history)", drive_links.get("p1_history", "https://drive.google.com"), use_container_width=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.link_button("🔑 Mark Schemes (ms_p1_p2)", drive_links.get("ms_p1_p2", "https://drive.google.com"), use_container_width=True)
            
        with c2:
            st.link_button("📜 Paper 2 (p2_history)", drive_links.get("p2_history", "https://drive.google.com"), use_container_width=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.link_button("🔑 Mark Schemes (ms_p3_p4)", drive_links.get("ms_p3_p4", "https://drive.google.com"), use_container_width=True)
            
        with c3:
            st.link_button("📜 Paper 3 (p3_history)", drive_links.get("p3_history", "https://drive.google.com"), use_container_width=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.link_button("📜 Paper 4 (p4_history)", drive_links.get("p4_history", "https://drive.google.com"), use_container_width=True)
...
DEVELOPER_NAME = "History & Computer Science Departments"
...
<div style="text-align: center; padding: 15px 0px; font-family: sans-serif;">
    ...
    <p style="margin: 5px 0 0 0; font-size: 0.85em; font-weight: 600; color: #384403;">💻 Developed for History Department ({SYLLABUS_CODE})</p>
</div>
"""
